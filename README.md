...
**None.** Deferred per the brief's guidance ("only if required behavior is done") in favor of polish on the required items: the sheet refactor, stale-async guard with deterministic test coverage, accessibility (skip-link, dialog ARIA, live region coordination), and detailed AI documentation.
